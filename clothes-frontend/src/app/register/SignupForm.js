...
export default function SignupForm() {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (responseData.status === 200) {
        setMessage("Registeration successfull, Now you can login");
        formRef.current.reset();
      } else if (responseData.status === 401) {
        setMessage("User already exists,Please use another email");
        formRef.current.reset();
      } else {
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="py-16">
      <div className="flex w-full max-w-sm mx-auto overflow-hidden bg-zinc-50 rounded-lg shadow-lg lg:max-w-4xl">
        <div
        ...
        <div className="w-full px-6 py-8 md:px-8 lg:w-1/2">
          <p className="mt-3 text-xl text-center text-gray-600 capitalize">Sign up page</p>
          <a
            href="#"
